...
- **CSV bulk import** of items via `POST /api/evaluations/items/import-csv?teamId=&mode=append|replace&position=all|gk|fp` (multer + papaparse, columns `category,subCategory,name,description`). The import is **position-bucket scoped**: each existing top category is classified as `common` (empty `targetPositions`), `gk` (`['GK']`), `fp` (DF/MF/FW only), or `mixed`. The import's `position` param maps to a target bucket (`all/common`, `gk`, `fp`). `replace` only deletes existing items inside the target bucket (other buckets are untouched — e.g. an FP replace never wipes GK items). `append` only reuses same-named top categories within the same bucket; a same-named top in a different bucket is left alone and a new top is created with the import's preset. `replace` refuses if any `Evaluation` rows still reference items in the target bucket. Newly-created top categories get `targetPositions` from the preset (`all`→`[]`, `gk`→`['GK']`, `fp`→`['DF','MF','FW']`); children inherit via `isPositionAllowed`. Deletion in replace mode runs in dependency order (leaves → subs → tops) to satisfy FK constraints. The CSV upload modal exposes this as 「対象ポジション（一式として登録）」 with three buttons 全選手共通／FP一式／GK一式 so a coach can register a GK set and an FP set as two separate uploads. Long-running transactions use `{ timeout: 60000, maxWait: 10000 }`. Auth: TEAM_MANAGER/COACH/operator.
...
- **Position targeting on items**: `EvaluationItem.targetPositions` (string[]) restricts who an item applies to (`GK / DF / MF / FW`). Inheritance: a leaf is "allowed" for a position iff itself and all ancestors are allowed (`isPositionAllowed`). The `/evaluation-items` modal exposes quick presets 「全選手共通」「フィールド (DF/MF/FW)」「GK専用」 above the per-position toggles so staff can register field-only or GK-only item trees without manual checkbox combos. The `/evaluation-items` page itself adds **view tabs 全て／全選手共通／FP／GK** (top categories classified by their `targetPositions`: empty=common, GK-only=gk, field-only=fp, mixed=other). Switching to FP or GK filters the tree and pre-fills the new 大項目 modal's `targetPositions` accordingly, so a coach can build GK and FP sets side-by-side without re-selecting presets each time. The 全選手共通 tab additionally surfaces a synthesized **「FPとGKで共通している項目」** section: client-side, leaves whose full path (`topName::subName::leafName`, trimmed) exists in BOTH an FP-classified and a GK-classified top category are listed read-only with FP×n / GK×n / 共通 badges. The tab's count includes natural common tops + overlap leaves so coaches can see at a glance that those items are effectively shared between sets.
...
- **Evaluation Matrix**: heatmap of player scores over time. Items are filtered per-player by `Player.position` using the same `isPositionAllowed` rule as elsewhere (inherited parent→child `targetPositions`) — a GK player only sees GK + common categories; an FP player only sees FP + common. Players with no `position` set fall back to showing all items. Applies to both the team matrix (`GET /api/evaluation-matrix/:teamId`) and the per-player matrix (`GET /api/evaluation-matrix/player/:playerId`).

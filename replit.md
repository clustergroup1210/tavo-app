...
- **Dashboard widget** (`TaskListWidget`): always reads `/api/tasks/my-tasks`. Open tasks sorted by dueDate with overdue/urgent coloring, type badge, click-to-navigate `targetUrl`, inline complete checkbox; "+作成" modal picks assignee kind (自分用メモ／スタッフ／選手／カテゴリー — order kept identical between the picker buttons; カテゴリー uses the bulk endpoint above).

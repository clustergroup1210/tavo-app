...
-   **Communication Tools**:
    -   **Calendar System**: Monthly view for practices, matches, meetings, and events, with color-coding, role-based access, and category-based targeting. Supports team-scoped, organization-scoped, and personal events. Events can target specific team categories (e.g., U-12, U-15), and players only see events matching their category. Personal events (`isPersonal: true`) are only visible to the creator and can be managed from both the Calendar page and MyPage's personal calendar section.

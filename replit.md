...
- **CSV bulk import** of items via `POST /api/evaluations/items/import-csv?teamId=&mode=append|replace&position=all|gk|fp` (multer + papaparse, columns `category,subCategory,name,description`). `replace` refuses if any `Evaluation` rows still reference existing items. The `position` query param applies a preset `targetPositions` to **newly-created top categories only** (`all`→`[]`, `gk`→`['GK']`, `fp`→`['DF','MF','FW']`); existing same-named top categories are untouched, and children inherit via `isPositionAllowed`. The CSV upload modal exposes this as 「対象ポジション（一式として登録）」 with three buttons 全選手共通／FP一式／GK一式 so a coach can register a GK set and an FP set as two separate uploads. Long-running transactions use `{ timeout: 60000, maxWait: 10000 }`. Auth: TEAM_MANAGER/COACH/operator.

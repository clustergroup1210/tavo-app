...
export default function PlayerDetail() {
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
          <div className="flex items-start gap-3 md:gap-4 md:flex-col md:items-stretch md:flex-shrink-0">
            <div className="flex gap-3 md:gap-4 flex-shrink-0">
              <div className="relative flex flex-col items-center">
                {player.photoUrl ? (
                  <img
                    src={player.photoUrl}
                    alt=""
                    className="w-20 h-20 md:w-40 md:h-40 rounded-xl md:rounded-2xl object-cover border-2 md:border-4 border-white/30 shadow-xl"
                  />
                ) : (
                  <div className="w-20 h-20 md:w-40 md:h-40 rounded-xl md:rounded-2xl bg-white/20 flex items-center justify-center border-2 md:border-4 border-white/30">
                    <UserCircle className="w-12 h-12 md:w-20 md:h-20 text-white/60" />
                  </div>
                )}
                <span className="hidden md:inline mt-1 text-xs text-white/80">プロフィール</span>
                {(isSelf || isCoachOrAdmin) && (
                  <label className="absolute -bottom-1 -right-1 md:bottom-5 md:-right-1 p-1 md:p-1.5 bg-white rounded-full shadow-lg cursor-pointer hover:bg-gray-100 transition-colors">
                    <Upload className="w-3 h-3 md:w-3.5 md:h-3.5 text-primary-600" />
                    <input
                      type="file"
                      accept="image/*"
              ...
              <div className="relative flex flex-col items-center">
                <div className="w-14 h-16 md:w-20 md:h-24 rounded-lg bg-white/20 flex items-center justify-center border-2 border-white/30 shadow-lg overflow-hidden">
                  {player.passportUrl ? (
                    <img
                    ...
                    <UserCircle className="w-7 h-7 md:w-8 md:h-8 text-white/60" />
                  )}
                </div>
                <span className="hidden md:inline mt-1 text-xs text-white/80">選手証</span>
                {(isSelf || isCoachOrAdmin) && (
                  <label className="absolute -bottom-1 -right-1 p-1 md:p-1.5 bg-white rounded-full shadow-lg cursor-pointer hover:bg-gray-100 transition-colors">
                    <Upload className="w-3 h-3 md:w-3.5 md:h-3.5 text-primary-600" />
                    <input
                      type="file"
                      accept="image/*"
            ...
            <div className="flex-1 min-w-0 md:hidden">
              <div className="flex items-center gap-2 mb-1">
                <span className="bg-white/20 rounded-lg px-2.5 py-0.5 backdrop-blur-sm text-xl font-bold leading-tight">
                  {player.number ? `No.${player.number}` : '-'}
                </span>
                {player.position && (
                  <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${positionColors[player.position] || 'bg-gray-500'}`}>
                    {player.position}
                  </span>
                )}
              </div>
              ...
              {player.nameRomaji && (
                <p className="text-xs text-white/80 truncate">{player.nameRomaji}</p>
              )}
      ...
      {activeTab === 'settings' && isCoachOrAdmin && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
          ...
          {player?.user ? (
            <div className="flex items-center justify-between p-4 bg-green-50 border border-green-200 rounded-lg">
              <div>
                ...
                <p className="text-sm text-green-600 break-all">{player.user.email}</p>
              </div>
              <button
                onClick={handleUnlinkUser}
                disabled={linkingUser}
                className="ml-3 flex-shrink-0 px-4 py-2 text-sm text-red-600 bg-white border border-red-300 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-50"
              >
                解除
              </button>
            </div>
